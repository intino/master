...
import io.intino.master.file.Launcher;
import io.intino.master.file.Master;
...
//		sleep(1000);
	...
	private void writeStats(List<LoadStats> stats) {
		File file = new File(folder, "stats-" + folder.getName() + ".tsv");
	...
	private LoadStats loadData5() {
...

		{
			...
			values.setAll(records.entrySet().stream().collect(toMap(Map.Entry::getKey, e -> compress(gson.toJson(e.getValue())))));
		}
		...
		stats.numEntries = values.size();
		stats.hzTotalMemMB = getTotalMemoryUsed();
	...
	private LoadStats loadData4() {
...

		{
			...
			values.setAll(records.entrySet().stream().collect(toMap(Map.Entry::getKey, e -> gson.toJson(e.getValue()))));
		}
	...
	private float getTotalMemoryUsed() {
		...
		return (valuesMem + subjectsMem + predicatesMem + reverseSubjectsMem + reversePredicatesMem) / 1024.0f / 1024.0f;
	}
